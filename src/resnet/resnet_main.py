...
tf.app.flags.DEFINE_string('train_data_path', 'debug/resnet/', 'train.records地址')
tf.app.flags.DEFINE_string('eval_data_path', 'debug/resnet/', 'eval.records地址')
...
tf.app.flags.DEFINE_string('train_dir', 'dist/resnet/train/', '训练输出')
tf.app.flags.DEFINE_string('eval_dir', 'dist/resnet/eval/', '验证输出')
tf.app.flags.DEFINE_integer('eval_batch_count', 1, '验证机一批样本数量')
tf.app.flags.DEFINE_bool('eval_once', False, 'Whether evaluate the model only once.')
tf.app.flags.DEFINE_string('log_root', 'dist/resnet/graph/', 'Directory to keep the checkpoints. Should be a ''parent directory of FLAGS.train_dir/eval_dir.')
