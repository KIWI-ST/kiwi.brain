...
HParams = namedtuple('HParams',
                     #一个batch内的图片个数,分类任务数目,最小的学习率,学习率
                     'batch_size, num_classes, min_lrn_rate, lrn_rate, '
                     #一个残差组内残差单元数量，是否使用boottleneck
...
class ResNet(object):
  ...
  def _build_model(self):
    ...
    if self.hps.use_bottleneck:
      ...
      res_func = self._residual_v1 #非bottleneck结构（包含两个3*3的卷积子层）
      filters = [16, 16, 32, 64]
  ...
  def _residual_v1(self, x, in_filter, out_filter, stride, activate_before_residual=False):
